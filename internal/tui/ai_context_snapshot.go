...
func aiContextGuide() map[string]string {
	return map[string]string{
		...
		"quote_stats":        "Compact quote stat labels rendered in the quote view for the active symbol. This also includes derived fields such as `Earnings Yield` and `Fwd Growth` when the needed inputs are available. `Earnings Yield` is the inverse of trailing PE when present, with a fallback to EPS divided by price. `Fwd Growth` is the market-implied next-period EPS growth inferred from trailing PE versus forward PE, and it represents expectations already embedded in the current price rather than a Blackdesk forecast.",
