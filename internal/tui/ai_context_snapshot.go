...
func aiContextGuide() map[string]string {
	return map[string]string{
		...
		"quote_stats":        "Compact quote stat labels rendered in the quote view for the active symbol. This also includes derived fields such as `Earnings Yield`, `Fwd Growth`, `N5Y Growth`, `Implied Return`, and `Implied Sharpe` when the needed inputs are available. `Earnings Yield` is the inverse of trailing PE when present, with a fallback to EPS divided by price. `Fwd Growth` is the market-implied next-period EPS growth inferred from trailing PE versus forward PE. `N5Y Growth` is the implied next-5-year EPS growth proxy derived from PE and PEG. `Implied Return` is `Earnings Yield + N5Y Growth`, so it should be read as a rough market-implied return proxy rather than a forecast. `Implied Sharpe` is `Implied Return / HV 252`, so it should be read as a simple return-to-risk heuristic rather than a formal Sharpe ratio.",
