...
func (m Model) renderAICenter(section, muted lipgloss.Style, width, height int) string {
	...
	if len(lines) == 0 {
		b.WriteString(renderWrappedTextBlock(muted, "No messages yet. Open this tab and start typing to ask the selected local AI about the current desk context.", width))
		if m.aiErr != nil {
